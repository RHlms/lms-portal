...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`Total custom fields on opp: ${fields.length}`);
    fields.forEach(f => {
      const val = f.value ?? f.fieldValue ?? f.fieldValueArray?.[0] ?? null;
      console.log(`ID: ${f.id} | key: ${f.key || f.fieldKey || 'none'} | val: ${val !== null ? String(val).substring(0, 60) : 'NULL'}`);
    });
    ...
    return res.status(200).json({
      ...
      debug: fields.map(f => ({
        id: f.id,
        key: f.key || f.fieldKey || null,
        value: f.value ?? f.fieldValue ?? f.fieldValueArray?.[0] ?? null
      }))
    });

  } catch (err) {
    console.error('File detail debug error:', err);
