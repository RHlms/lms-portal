...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!contactUpdate.ok) throw new Error(`Contact update failed: ${contactUpdate.status}`);
    ...
    if (!oppUpdate.ok) throw new Error(`Opportunity update failed: ${oppUpdate.status}`);
